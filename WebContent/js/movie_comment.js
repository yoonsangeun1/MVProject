...
function carouselInit(height, t) {
	/** height: 캐러셀 높이, t: 사진 전환 간격 */
	
	time = t;
	$("#carousel_section").height(height); /** 캐러셀 높이 설정 */
...
function carousel() {
	...
	caInterval = setInterval(function() {
		...
		if (currentIndex == (carouselCount - 1)) {
			...
			/** 마지막 슬라이드가 넘어갈 땐 처음 슬라이드가 보이도록 함 */
	...
	}, time); // 우선 초기 설정 함수를 만들어서 변수 선언과 함수들을 선언해주었다.
